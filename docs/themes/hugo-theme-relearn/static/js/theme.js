...
function switchTab(tabGroup, tabId) {
  ...
  allTabItems && allTabItems.forEach(function (e) {
    e.classList.remove('active');
    e.removeAttribute('tabindex');
  });
  targetTabItems && targetTabItems.forEach(function (e) {
...
function initMermaid(update, attrs) {
  ...
  if (is_initialized) {
    mermaid.initialize(Object.assign({
      ...
      "startOnLoad": false
    }, window.relearn.mermaidConfig, {theme: attrs.theme}));
    mermaid.run({
...
function initMenuScrollbar() {
  ...
  window.addEventListener('resize', function () {
    ...
    psm && setTimeout(function () {
      psm.update();
    }, 10);
    psc && setTimeout(function () {
      psc.update();
    }, 10);
  });
