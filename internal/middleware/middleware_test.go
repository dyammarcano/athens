...
func newTestFilter(filterFile string) (*module.Filter, error) {
	...
	f.AddRule("github.com/dyammarcano/athens/", nil, module.Direct)
...
func Test_FilterMiddleware(t *testing.T) {
	...
	path := "/github.com/dyammarcano/athens/@v/list"
	res := w.JSON(path).Get()
	r.Equal(http.StatusSeeOther, res.Code)
	r.Equal(conf.GlobalEndpoint+"/github.com/dyammarcano/athens/@v/list", res.HeaderMap.Get("Location"))
...
func (suite *HookTestsSuite) TestHookOnList() {
	...
	suite.w.JSON("/github.com/dyammarcano/athens/@v/list").Get()
